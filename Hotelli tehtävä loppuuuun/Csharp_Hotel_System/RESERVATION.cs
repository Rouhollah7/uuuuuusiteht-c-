...
namespace Csharp_Hotel_System
{
    class RESERVATION
    {
        ...
        public bool addReservation(int number, int clientId, DateTime dateIn, DateTime dateOut)
        {
            ...
            String insertQuery = "INSERT INTO `reservations`(`roomNumber`, `clientId`, `DateIn`, `DateOunt`) VALUES (@rnm,@cid,@din,@dout)";
        ...
        {
            MySqlCommand command = new MySqlCommand();
            String editQuery = "UPDATE `reservations` SET `roomNumber`=@rnm,`clientId`=@cid,`DateIn`=@din,`DateOunt`=@dout WHERE `reservId`=@rvid";
